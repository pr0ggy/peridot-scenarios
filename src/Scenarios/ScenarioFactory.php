...
/**
 * Handles creation and configuration of new scenarios based on given setup and teardown
 * entities.
...
class ScenarioFactory
{
     ...
     * If the $setup argument is already callable, it is returned as-is.  If the $setup
     * arg is given as a k/v map, it is assumed that the map simply defines field names
     * and values that should be set in the scenario test context, so a closure which
     * handles this is returned.
     *
     * @param  callable|array $setup
     * @return callable
     * @throws TypeError if the given $setup arg is not an array or callable
     */
    ...
    /**
     * Accepts a k/v map an returns a closure which binds each k/v pair to a context field
     * and value. The context ($this) will be bound to test's scope when the test is run,
     * meaning the new fields and values will be accessible via '$this' within a test's
     * definition.
     *
     ...
     * If the $teardown argument is already callable, it is returned as-is.  If it is null,
     * then a no-op closure is returned.  Any other value results in a TypeError.
     *
     * @param  callable|null $teardown
     * @return callable
     * @throws TypeError if any $teardown argument is given that is neither null nor callable
     */
    protected function getScenarioTeardownAsCallable(callable $teardown = null)
    {
